...
class DataLoaderTest extends AnyFlatSpec with GivenWhenThen {
  ...
  it must "save and load custom decks correctly" in {
    ...
    val customDeck: Seq[DeckCard] = DeckCard("base-1", Some(SetType.Base), "myPokemon", "rare", 1) ::
      DeckCard("base-2", Some(SetType.Base), "myPokemon2", "rare", 1) ::
      DeckCard("base-3", Some(SetType.Base), "myPokemon2", "rare", 1) :: Nil
