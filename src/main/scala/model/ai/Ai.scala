...
object Ai extends Thread with Observer {
  ...
  private def doTurn(): Unit = {
    ...
    if (!GameManager.isBenchLocationEmpty(0, opponentBoard) && opponentBoard.activePokemon.get.retreatCost.size <= opponentBoard.activePokemon.get.totalEnergiesStored) {
      try {
        ...
        case exception : InvalidOperationException => println("can't retreat")
        case _ =>
      }
    }
  ...
  override def run() {
    try {
      while (true) {
        val event: Event = Ai.waitForNextEvent()
        event match {
          ...
          case _: Event.PokemonKO => checkForKo()
          case _ =>
        }
      }
    }
